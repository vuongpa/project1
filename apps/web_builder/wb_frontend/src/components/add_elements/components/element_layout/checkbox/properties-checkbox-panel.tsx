...
import { CheckboxPropertiesDefaults } from "./checkbox-properties";
import { alpha } from "@mui/material/styles";
import SettingsIcon from "@mui/icons-material/Settings";

interface CheckboxPropertiesProps {
  properties: CheckboxPropertiesDefaults;
  onPropertyChange: (newProperties: Partial<CheckboxPropertiesDefaults>) => void;
  size?: "small";
}

export const CheckboxProperties: React.FC<CheckboxPropertiesProps> = ({ properties, onPropertyChange, size = "small" }) => {
  const [expandedSection, setExpandedSection] = useState<string | null>(null);

  const handleChange = <K extends keyof CheckboxPropertiesDefaults>(
    key: K,
    value: CheckboxPropertiesDefaults[K]
  ) => {
    onPropertyChange({ [key]: value });
  };

  const sections = [
    { id: "checkbox", label: "Checkbox", details: ["label", "value", "name", "checked", "disabled"] },
    { id: "appearance", label: "Appearance", details: ["color", "fontSize", "padding", "margin", "backgroundColor", "hoverBackgroundColor", "checkedBackgroundColor", "border"] },
  ];
          ...
          >
            ...
            >
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                ...
                {section.details.includes("disabled") && (
                  <Box display="flex" alignItems="center" gap={1}>
                    <Typography variant="caption" sx={{ minWidth: 80, color: alpha("#ffffff", 0.7), fontSize: "0.75rem" }}>
                      Disabled
                    </Typography>
                ...
                {section.details.includes("hoverBackgroundColor") && (
                  <Box display="flex" alignItems="center" gap={1}>
                    <Typography variant="caption" sx={{ minWidth: 80, color: alpha("#ffffff", 0.7), fontSize: "0.75rem" }}>
                      Hover Bg
                    </Typography>
                    <TextField
                      type="text"
                      value={properties.hoverBackgroundColor || "#ffffff"}
                ...
                {section.details.includes("checkedBackgroundColor") && (
                  <Box display="flex" alignItems="center" gap={1}>
                    <Typography variant="caption" sx={{ minWidth: 80, color: alpha("#ffffff", 0.7), fontSize: "0.75rem" }}>
                      Checked Bg
                    </Typography>
                    <TextField
                      type="text"
                      value={properties.checkedBackgroundColor || "#ffffff"}
                      onChange={(e) => handleChange("checkedBackgroundColor", e.target.value)}
                      variant="outlined"
                      size={size}
                      InputProps={{
                        startAdornment: (
